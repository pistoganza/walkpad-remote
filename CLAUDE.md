...
```bash
# Сервер с no-cache заголовками (скрипт /tmp/w2server.py)
python3 /tmp/w2server.py
# Или простой:
cd ~/ProjectsCode/w2 && python3 -m http.server 8092
...
- Start / Stop
- Пресеты скорости: 1.0 / 1.5 / 2.0 / 2.5 / 3.0 км/ч (автостарт)
...
- Инструкция подключения на русском (встроена в страницу)
